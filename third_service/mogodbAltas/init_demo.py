#!/usr/bin/env python
# -*- coding: utf-8 -*-
# python_demo.init_demo.py
# @Calendar: 2025-04-20 01:56
# @Time: 1:56
...
def list_comment():
    uri = "mongodb+srv://kiramario:JVDohfFRpTuDzPlF@cluster0.gwmkvqe.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
    ...
    result = comments_collection.find()

    if result:
        for doc in result:
            comment_text = doc['text']
            print(f"txt = {comment_text}")
...
def de():
    ...
    recipe_documents = [
        {"name": "elotes", "ingredients": ["corn", "mayonnaise", "cotija cheese", "sour cream", "lime"],
         "prep_time": 35},
    ...
    if result:
        for doc in result:
            ...
            print(
                "%s has %x ingredients and takes %x minutes to make." % (my_recipe, my_ingredient_count, my_prep_time))
