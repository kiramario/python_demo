...
def get_secrets():
    with open(expanduser('~/chat-robot.env')) as f:
        secrets = json.load(f)
    return secrets["tensent"]["sms"]

sms_secrets = get_secrets()
...
def run():
    try:
        ...
        cred = credential.Credential(sms_secrets["secretId"], sms_secrets["secretKey"])
        ...
        req.PhoneNumberSet = ["+8615681565885"]
